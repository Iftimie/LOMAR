...
first_layer = np.arange(10,100,10)
second_layer =  np.arange(5,30,5)
sizes =[]
...
bestConfig = 0
for config in sizes:
    clf = MyTFModel(scalerY=scalerY,input_size=309,output_size=3,sizes=config)
    scores = cross_val_score(clf, X_train1, Y_train1, cv=5)
    print(scores,np.mean(scores),config)
...
print ('\x1b[0;33;45m',best,bestConfig)
